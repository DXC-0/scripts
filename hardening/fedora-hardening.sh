...
firewall-cmd --permanent --zone=block --add-port=$RANDOM_PORT/tcp
firewall-cmd --reload
...
semanage port -a -t ssh_port_t -p tcp $RANDOM_PORT 2>/dev/null || \
semanage port -m -t ssh_port_t -p tcp $RANDOM_PORT || true

echo "Simple permission check"
...
check "systemctl is-active sshd | grep -q active" "SSH: service active"
...
check "systemctl is-active firewalld | grep -q active" "Firewalld: firewall active"
check "firewall-cmd --get-default-zone | grep -q 'block'" "Firewalld: default zone is block"
check "firewall-cmd --zone=block --list-ports | grep -q '$RANDOM_PORT/tcp'" "Firewalld: SSH port allowed in block zone"
...
check "getenforce | grep -q 'Enforcing'" "SELinux: enforcing mode"
check "sestatus | grep -q 'enabled'" "SELinux: enabled"
check "sestatus | grep -q 'targeted'" "SELinux: targeted policy active"
check "semanage port -l | grep -q $RANDOM_PORT" "SELinux: SSH port registered"
